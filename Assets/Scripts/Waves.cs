...
public class Waves : MonoBehaviour
{
    ...
    {
        // Ajuste para leer el espacio global escalado correctamente
        var scale = new Vector3(1 / transform.lossyScale.x, 0, 1 / transform.lossyScale.z);
        var localPos = Vector3.Scale((position - transform.position), scale);

        // Compensamos el centrado en el cálculo de la altura
        localPos.x += Dimension / 2f;
        localPos.z += Dimension / 2f;
    ...
    private Vector3[] GenerateVerts()
    {
        ...
        // MODIFICACIÓN QUIRÚRGICA: Restamos la mitad para centrar el pivote perfectamente
        float offset = Dimension / 2f;

        for (int x = 0; x <= Dimension; x++)
            for (int z = 0; z <= Dimension; z++)
                verts[index(x, z)] = new Vector3(x - offset, 0, z - offset);
    ...
    void Update()
    {
        ...
        for (int x = 0; x <= Dimension; x++)
        {
            for (int z = 0; z <= Dimension; z++)
            {
                ...
                verts[index(x, z)] = new Vector3(x - (Dimension / 2f), y, z - (Dimension / 2f));
            }
        }
        Mesh.vertices = verts;
        Mesh.RecalculateNormals();
