...
public class WaterBoat : MonoBehaviour
{
    ...
    public void FixedUpdate()
    {
        ...
        // Cambiamos Input.GetKey por la lectura nativa del New Input System para Unity 6
        // Nota: Lee directamente el teclado físico sin importar el mapa de acciones
        var teclado = UnityEngine.InputSystem.Keyboard.current;

        if (teclado != null)
        {
            if (teclado.aKey.isPressed) steer = 1;
            if (teclado.dKey.isPressed) steer = -1;
        }
        ...
        //forward/backward power con el nuevo sistema
        if (teclado != null)
        {
        ...
        {
            if (teclado != null && (teclado.wKey.isPressed || teclado.sKey.isPressed))
                ParticleSystem.Play();
            else
